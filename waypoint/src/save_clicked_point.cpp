...
PointSaver::PointSaver() : nh("~") {
    ...
    point_sub = nh.subscribe<geometry_msgs::PointStamped>("/clicked_point", 1, &PointSaver::savePointToFile, this);
...
void PointSaver::executeCallback(const std_msgs::Int16::ConstPtr& msg) {
    if (msg->data == 1) {
        ...
        ROS_INFO("Received signal to execute Point Saver.");
    } else {
        subscribe_sip_node = false;
        ROS_INFO("Received signal to stop Point Saver execution.");
    }
}
